...
- [x] **Feature:** bundled CC0 music library — six background tracks ship inside the installers, picked from the timeline's audio menu and laid on the existing music lane with a quiet default gain, fades and looping. Only **CC0 1.0** is eligible: a "free for commercial use" licence (Pixabay, Mixkit, Bensound, Uppbeat) forbids redistributing the file, which is exactly what bundling does, and CC-BY would push an attribution obligation all the way into the user's exported video. Every track carries an archived copy of its licence statement and a pinned SHA-256, enforced by `npm run music:check` in its own CI job. "More music" is a link to [the docs page](website/docs/music.md), not a downloader — we host nothing beyond what already ships.
...
- **2026-09-15** — added the bundled CC0 music library. The feature itself was small because music tracks already existed end to end (schema, lane, inspector, preview, native mix, all from #350); what the work actually bought is the licensing spine around them: a provenance manifest with archived licence captures, a build-time guard that refuses anything but CC0, a notices section that reaches the user because `"!*.md"` strips every README from the package, and a docs page that says plainly what a user may do with the tracks — including the Content ID caveat, which is a real risk for public-domain music and which no vendor has any reason to mention. Tracks ship byte-for-byte as published upstream rather than re-encoded, so the pinned digest stays checkable against the source.
