...
Recordly uses native ScreenCaptureKit-based capture helpers on macOS and keeps platform capture behind helper boundaries, with Windows handled by a separate WGC helper. The OpenScreen macOS port should follow the same separation without copying Recordly implementation details directly.
