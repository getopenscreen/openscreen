...
- [ ] Opening an existing video file works **from the launch window** — still unverified as literally stated: there is no direct "open video" button on the HUD/launch window (confirmed via `grep` — no `launch-open-video-button` or similar exists in `LaunchWindow.tsx`). `tests/e2e/windows-native-checklist.spec.ts` references `hudWindow.getByTestId("launch-open-video-button")` and `"launch-open-project-button")`, neither of which exists anywhere in the component tree — these two tests would fail immediately if ever run on real Windows; they've never caught this because the spec self-skips off-Windows and CI is Linux-only, so it's never actually executed. Filed as [issue #130](https://github.com/getopenscreen/openscreen/issues/130)
- [x] Opening an existing video file works via **HUD → "Open Studio" → editor empty state → "Import Video File…"** (the actual current path, distinct from the item above) — confirmed working end-to-end in the 2026-07-19c macOS pass: imported an existing recording this way and it loaded correctly into the editor
...
- [ ] System-audio-only recording produces audible system audio on playback — a waveform spike is evidence of a captured signal, not proof it's audible on playback, and this pass had no audio-playback capability to confirm that distinction, so leaving unchecked. Recorded in the 2026-07-19c macOS pass with system-audio on / mic off / webcam off, playing `afplay /System/Library/Sounds/Glass.aiff` partway through: the editor's timeline waveform shows a distinct amplitude spike exactly at the moment the sound played (and is flat elsewhere) — solid capture-side evidence, but audibility itself is unverified
