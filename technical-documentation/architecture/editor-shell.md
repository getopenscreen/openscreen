...
| **Export dialog** | `src/components/ai-edition/ExportDialog.tsx` | Named destinations (Web / YouTube, Social, Studio, README GIF) that set format / quality / frame-rate / codec / size together, all of which stay under **Advanced**. Both formats go through the native compositor with the same clip list (`resolveVisibleClips`) and scene (`buildSceneDescription`): `exportGifNative` for GIF, `exportMultiNative` for MP4. |
