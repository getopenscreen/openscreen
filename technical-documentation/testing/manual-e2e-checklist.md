...
| 2026-09-07 | locally built and installed `v1.11.0-rc.1` from `release/1.11.0-rc.1` @ `027f9e3c` (rebased on `main`, includes #618). Apple Silicon DMG, Developer ID signed; local development artifact, not published or notarized. Verified payload includes executable `ffmpeg` 8.1.2, compositor, SCK helpers, Whisper/ONNX and FFmpeg dylibs; `codesign --verify --deep --strict` passes. | macOS 26.5, M1 | Partial — STT blocker fixed | **No-cache STT proof:** opened a project whose asset had zero transcript segments, then used the installed build without `OPENSCREEN_FFMPEG_PATH`. UI progressed `Transcribing 0%` → `Transcript ready`; logs show Whisper Metal loading, native audio extraction and `[stt] done on whispercpp-metal` for the 0.2 s AAC asset (1.9 s inference). The project subsequently contains a new transcript asset id, language, one segment and one word. This rules out the cached transcript masking the former silent ffmpeg failure. A 65.5 s cached-project regenerate also completes normally (19.3 s / 0.29 RTF), but is supporting rather than decisive evidence. An asset with no decodable audio now exposes `NoAudioTrackError` in the UI instead of the previous `FfmpegUnavailableError`. **Continued editor slice:** imported a local synthetic WAV; waveform, audio inspector, mute and loop all worked. Native Edit-menu undo/redo correctly changed then restored a Zoom modifier (keyboard shortcuts remain inconclusive under this driver). Theme toggled both directions; Cursor inspector toggles and controls were reachable; Edit Clip dialog opened with source preview and crop fields. Chat panel and AI settings opened, but no request was sent because the loaded project contains user recording/transcript. Capture/restart/cancel remains skipped: the fresh install lacks macOS Screen Recording permission; changing that OS security setting was not performed. Webcam/voice-over remain unavailable: this Mac reports no camera or microphone. Remaining prior gaps: clip drag/reorder/copy-paste, modifier anchoring under trims, second mixed-scale display, and Windows/Linux. |
|      |             |          |           |       |
|      |             |          |           |       |
