...
| 2026-09-07 | locally built and installed `v1.11.0-rc.1` from `release/1.11.0-rc.1` @ `027f9e3c` (rebased on `main`, includes #618). Apple Silicon DMG, Developer ID signed; local development artifact, not published or notarized. Verified payload includes executable `ffmpeg` 8.1.2, compositor, SCK helpers, Whisper/ONNX and FFmpeg dylibs; `codesign --verify --deep --strict` passes. | macOS 26.5, M1 | Partial — STT blocker fixed; 1 system-audio defect | **No-cache STT proof:** opened a project whose asset had zero transcript segments, then used the installed build without `OPENSCREEN_FFMPEG_PATH`. UI progressed `Transcribing 0%` → `Transcript ready`; logs show Whisper Metal loading, native audio extraction and `[stt] done on whispercpp-metal` for the 0.2 s AAC asset (1.9 s inference). The project subsequently contains a new transcript asset id, language, one segment and one word. This rules out the cached transcript masking the former silent ffmpeg failure. A 65.5 s cached-project regenerate also completes normally (19.3 s / 0.29 RTF), but is supporting rather than decisive evidence. An asset with no decodable audio now exposes `NoAudioTrackError` in the UI instead of the previous `FfmpegUnavailableError`. **System audio defect:** with System audio ON and microphone OFF, recorded the entire screen for 20.38 s while a local 440 Hz / 48 kHz tone was actively playing through `afplay` (exit 0). The produced MP4 is healthy (3840×2160 H.264 plus AAC-LC 48 kHz stereo) but its audio is digital silence: AAC averages 2 kb/s and `volumedetect` reports mean/max `-91.0 dB` over 1,957,760 samples. Two shorter controls (0.55 s and 3.23 s) show the same silent track. Capture itself, source selection and clean stop work; this is an audio-content failure, not missing track creation. **Continued editor slice:** imported a local synthetic WAV; waveform, audio inspector, mute and loop all worked. Native Edit-menu undo/redo correctly changed then restored a Zoom modifier (keyboard shortcuts remain inconclusive under this driver). Theme toggled both directions; Cursor inspector toggles and controls were reachable; Edit Clip dialog opened with source preview and crop fields. Chat panel and AI settings opened, but no request was sent because the loaded project contains user recording/transcript. Webcam/voice-over remain unavailable: this Mac reports no camera or microphone. Remaining prior gaps: restart/cancel, clip drag/reorder/copy-paste, modifier anchoring under trims, second mixed-scale display, and Windows/Linux. |
| 2026-09-07 | Correction to the local `v1.11.0-rc.1` retest above. | macOS 26.5, M1 | Pass — system audio | **Retracts the synthetic-tone defect.** The `afplay` control captures digital silence on this machine, but is not representative of the ScreenCaptureKit route. A subsequent user-driven browser/YouTube capture with System audio ON produces a 99.46 s H.264 + AAC-LC 48 kHz stereo MP4 whose audio measures mean `-29.3 dB`, max `-2.6 dB`; the editor waveform visibly matches the signal. System-audio capture, source selection and clean stop pass. |
|      |             |          |           |       |
|      |             |          |           |       |
