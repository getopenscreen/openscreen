...
| 2026-09-24 | installed `v1.13.0-rc.4` — CI-built DMG `Openscreen-macOS-Apple-Silicon-1.13.0-rc.4.dmg` from the GitHub Release (tag `187e0d8d`), Developer ID signed + notarized (`spctl`: accepted, Notarized Developer ID), About reports `1.13.0-rc.4 (dmg)`; native payload one uniformly stamped set (all `darwin-arm64` files 2026-09-22 10:05) | macOS 26.5 (25F71), M1 Mac mini, one 3840×2160 display; **no camera and no microphone attached** | Partial — 8 defects, none blocking in what ran | **Full-file attempt with real OS mouse/keyboard; everything not run is listed.** **Passed:** launch/HUD (layout toggle, drag follows pointer and stays dropped, language menu, minimize → tray refocus, close/relaunch, no orphan processes after Quit); source picker (Screens/Windows tabs); record with countdown, pause freezes timer, resume, restart resets, cancel opens no editor; stop from HUD **and** from tray *Stop Recording* (tray icon turns red while recording); recording 3840×2160 H.264 + AAC 48 kHz filled edge to edge (measured: 0 dark px on all four borders); system audio captured (−10.9 dB mean); relaunched **without** the protection flag → `[content-protection] OFF … macOS 26.5.0` logged, HUD visible on screen and **absent from the recorded frame**. Editor: rename, Media/Edit/Rec modes, chat panel, play/pause, seek bar + ruler seek, trim edges + playback skips the trim, zoom levels/3D camera (Off, 3 fixed angles, orbit) in preview, auto zooms from clicks, speed presets + custom 2.5× + 150 → "can't go higher than 100×" toast, text annotation (text/colour/background/typewriter), arrow (direction/stroke), blur (mosaic/gaussian/oval), delete + ⌘Z/⌘⇧Z, second clip by drag from Media, reorder (modifiers stay anchored to their clip, checked in the document), Edit Clip crop (↓ on W = 99.97395 = one px of 3840, partial "8" stays editable, 80 % crop reframes), clip delete leaves other clip's modifiers. Transcript: local Whisper on `whispercpp-metal` (4.5× RT), "no data leaves the device" notice, word click seeks, cue highlight, Backspace cuts a word (trim created), double-click correction propagates to captions and to the translation, **typing between words is inert in the packaged build**; captions on/off, Top/Left anchor with relabelled sliders, translation FR via provider then delete → original back. Chat (DeepSeek): 2-op turn applied and undone/redone as one unit, "cut the silences" explained instead of guessing, rewind cancel leaves everything, rewind confirm rolls back. Imported audio: pill named after the file, gain/fades/mute/loop controls, **exported** gain exactly −4.0 dB, fades at the track's own edges (1.45 s ramps), loop fills to the end with no seam, mute → track absent (noise floor), recording audio unchanged under the bed, track not time-stretched by a 2× region and cut at the last frame without pulling the fade forward, **still audible in an export after an app restart**. Export: MP4/GIF options + dimensions, progress reaches 100 %, frame math exact (take 2: 1086 − 55.5 trim − 18.5 speed = 1012.1 expected, **1013** written; take 1: 1032.6 expected, **1032** written; audio = video duration in both at 30 fps), no silence hole at speed boundaries (`silencedetect`, the one gap maps to a pause already in the source), caption plate left edge **x = 192** for `insetX 10` and top edge y = 16 for 1.5 % (measured on PPM), annotation/captions do not follow a zoom, 3D tilt + text + oval blur + cursor all in exported frames; GIF cancel returns to options with no partial file, full GIF 1280×720, 508 frames, infinite loop, opens in Preview; MP4 opens in QuickTime. Settings: background image/colour/gradient, padding/roundness/shadow, cursor hide + style, light theme (gradient picker + dialogs follow), French UI incl. app menu and wordmark menu, custom shortcut (T→Y works, restored), ⌘S/⌘O dialog. App menu About / Save Diagnostics (valid JSON bundle) / Check for Updates → "up to date (1.13.0-rc.4)"; same from the HUD panel (which shows version, "No microphone found", "No camera found"); tray Update Settings has the 3 modes and the pick survives a restart; editor bounds (resize, maximize, un-maximize persisted; zero-size + `maximized:"yes"` file → sane default window). **Defects:** (1) *Preview only* — a text annotation with an animation renders inconsistently **during playback** in the native preview (blank, partial "R", or complete across repeated plays of the same span) while paused seeks and the export are correct; related: the first paused frame after creating an annotation or reopening a project is sometimes stale until the next seek. (2) Clicking the editor behind the Export modal while the native *Save Exported Video* panel is up sends that panel behind the editor: invisible, modal stuck on "Starting…", its Cancel inert — recovered only by pressing the panel's Cancel through Accessibility; reproduced twice. (3) HUD dialogs (Check for Updates from the HUD panel) grey a ~900×700 area of the desktop — **already fixed on `main` by #733, not in rc.4: cherry-pick candidate**. (4) GIF delay is truncated, not rounded (`crates/compositor/src/gif_export.rs:231`, `100 / fps`): 15 fps → 6 cs → plays at 16.7 fps, 30.48 s for a 33.87 s programme (+11 %, not the ~4 % the comment allows); 30 fps has the same error. (5) Hovering a Backspace-cut transcript word shows no *Restore* button (undo and the trim inspector still restore). (6) Whisper appends a hallucinated repeat of the whole transcript over the trailing ~8 s of silence, stacked at 34.4 s. (7) The "frame-stepped and muted" hint (`speed.previewFrameSteppingHint`) is rendered nowhere at 20×. (8) Cosmetic: the shortcuts dialog lists Undo/Redo as Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y on macOS. **Checklist drift, not defects:** no loop or fullscreen transport control exists (`transport.loop` key unused); blur has no intensity/block-size control (fixed 12/12) and freehand only for legacy docs; projects autosave, so the unsaved indicator and Save/Discard/Cancel prompt never appear; the app menu puts Save Diagnostics between About and Check for Updates. **Skipped — no hardware:** microphone (mic-only, mic+system balance, HUD mic prompt), webcam PiP/layouts/shapes/full-camera/shrink-on-zoom, AI camera background (no webcam track), voice-over, second display at another scale factor; **single Space:** record while switching Spaces; **driver limits:** Ctrl/Shift/middle-mouse timeline zoom/pan (modifier+scroll never lands), Esc on menus; **not run:** chat history/rename/delete/new-conversation switch, compact context, copy message, API-key entry and device-flow panels, auto-enhance AI path, update download/install/restart flows (no newer release), old-project migration and legacy caption annotations, region merge/repel/cross-boundary split, clip copy/paste, transcript silence-span trim, re-transcription, word insertion (dev-only), notes window exclusion, Intel/Store/Windows/Linux blocks. Main-process logs clean (no error lines) across both launches; defect screenshots kept locally. |
| 2026-09-24 | installed `v1.13.0-rc.6` — CI-built DMG `Openscreen-macOS-Apple-Silicon-1.13.0-rc.6.dmg` from the GitHub Release (tag `6eb86b17`, build run 35990025043), Developer ID signed + notarized (`spctl`: Notarized Developer ID); native payload one uniformly stamped set (34 files, 2026-09-24 13:10). rc.6 = rc.5 (#733 backport) + cherry-picks of #748, #749, #750 (2 commits), #719 (2 commits), #666 | macOS 26.5 (25F71), M1 Mac mini, one 3840×2160 display; no camera, no microphone | Pass — risk-scoped slice, no defect | **Scoped to what rc.5→rc.6 changed, not a full-file run** (the full pass is the rc.4 row). Real OS mouse/keyboard; every level, duration and frame claim measured on exported files. **#733 HUD dialogs:** HUD device-settings → Check for Updates shows "up to date (1.13.0-rc.6)" with **no grey rectangle** over the desktop (rc.4: ~900×700 greyed). **#743 file panels (widest blast radius — every save/open dialog):** Export MP4 → *Save Exported Video* is now a sheet on the editor; three clicks on the editor behind the modal leave it attached and visible (rc.4: panel fell behind, modal stuck on "Starting…"); Cancel returns the modal to its options. Open Project → *Browse files…* and the audio-import picker (`m`) also open as sheets on the editor and complete. **#744 GIF timing:** GUI GIF export 15 fps / Medium: 517 frames, delays 7/6 cs alternating (345×7 + 172×6), playback **34.47 s = frames/15 exactly**, against 34.40 s for the MP4 of the same project (rc.4: 30.48 s for a 33.87 s programme); CLI GIF 15 fps: 543 frames, 36.20 s = 543/15. **#747:** Keyboard Shortcuts → Fixed shows Undo "⌘ + Z" and Redo "⌘ + ⇧ + Z / ⌘ + Y". **#719 CLI export of v2 projects:** three hand-written v2 projects over a real take — with `cropRegion {0.25,0.25,0.5,0.5}` the CLI export on **rc.4 ignored the crop (full frame)**, on rc.6 it renders the centre crop; `cursorSize: 6` renders a cursor twice the default (3.0) size, compared frame-for-frame against the same project without the key; an out-of-range crop `{-0.2,0.5,0.6,0.8}` exports without error (clamped); the v2 crop project also opens in the GUI with the crop applied. **#666 imported-audio gain:** the Output level slider now goes well below −12 dB (set −37.5 and −39.0 dB); exported level of the 1 kHz bed by single-bin lock-in over 11–13 s: **−31.1 dBFS at −4 dB (rc.4) → −65.9 dBFS at −39 dB (rc.6), Δ 34.8 dB for 35 dB requested**, muted floor −96.2 dBFS. **Smoke:** record from the editor's Rec stage (tray icon red, stop from tray *Stop Recording*) → editor opens on the 13.23 s 3840×2160 H.264 + AAC take with an auto zoom → add a trim (0.68 s) → Export MP4 1080p30: 12.60 s (13.23 − 0.68 = 12.55 expected, within 2 frames). Main-process log: no error lines. **Not covered:** everything outside this slice (see the rc.4 row); #743 on Windows/Linux (the helper keeps owning the HUD there — unit-tested only); the native-bridge project save path was exercised only through Open; webcam/mic still untestable on this machine. Open, not in rc.6: #742 (animated annotation during playback, preview only), #745 (not reproduced), #729 (Whisper hallucination on trailing silence). |
| 2026-09-24 | `claude/zoom-speed-adjustment-fb8abd`, the zoom-pace PR for #683, on `d951e532`; `compositor_view.node` rebuilt from its first commit `cc88c423` (the later motion-blur fix at speed boundaries does not touch these measurements, taken with motion blur off) by `npm run build:native:compositor` (2026-09-24 22:17), ffmpeg DLLs hash-identical to the vendored SDK; renderer rebuilt with `npm run build-vite` | Windows 11 26200 | Partial — no defect | **Zoom pace and speed regions, through the app's real export (`electron . export <project> -o <mp4>`).** Source: synthetic 1920×1080 clip, a 160 px white square centred. Two legacy projects: `plain` at 1× with a 1.25× zoom on [2 s, 3 s] and a 5× zoom on [6 s, 8 s]; `sped` with the whole clip at 4× and the same zooms at 4× the source times. Zoom scale measured on every frame of the 60 fps exports (area-weighted width of the square). New addon: `plain` and `sped` match frame for frame within a maximum scale difference of 0.0001;\|Δscale\| 0.0001); 1.25× moves 2→98 % in 0.53 s, peak 0.9 e-folds/s; 5× in 1.12 s, peak 2.8 e-folds/s; each zoom reaches 98 % before its region starts and leaves when it ends. Same app build pointed at the 2026-09-13 addon (`OPENSCREEN_COMPOSITOR_VIEW_NODE`): 5× peaks at 13 e-folds/s at 1× and 31 at 4×, where the whole zoom-in takes 0.18 s. **Not covered:** the GUI editor preview (same `plan_frame` as the export, not driven), Full Camera under a speed region and chained zooms (unit-tested only), macOS and Linux rendering. |
