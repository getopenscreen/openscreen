...
The same recorded movement drives the cursor in the export. The Cursor facet shows or hides it and sets its size, smoothing, motion blur and click bounce. The cursor path is smoothed the same way for the preview and the export, so the preview matches the file.
